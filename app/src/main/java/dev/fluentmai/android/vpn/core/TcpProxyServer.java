...
public class TcpProxyServer implements Runnable {
    ...
    static InetSocketAddress getDestAddress(SocketChannel localChannel) {
        ...
        if (session != null) {
            if (ProxyConfig.Instance.needProxy(session.RemoteIP) ||
                    ProxyConfig.Instance.needProxy(session.RemoteHost)) {
                return new InetSocketAddress(
                        CommonMethods.ipIntToInet4Address(session.RemoteIP),
    ...
    void onAccepted(SelectionKey key) {
        ...
        try {
            ...
            Log.i(Constant.TAG, "TcpProxy accepted local=" + localChannel.socket().getLocalPort() +
                    " remotePortKey=" + localChannel.socket().getPort());
            localTunnel = TunnelFactory.wrap(localChannel, m_Selector);

            InetSocketAddress destAddress = getDestAddress(localChannel);
            if (destAddress != null) {
                Log.i(Constant.TAG, "TcpProxy dest=" + destAddress.getHostString() + ":" + destAddress.getPort());
