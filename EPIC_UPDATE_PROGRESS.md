...
## 2026-07-15：Android 产品收束与纠偏回补

本节是上一轮 Epic 之后的 Android 收束结果；上文中的测试数量、页面结构和“最终结果”仅保留为当时历史快照。完整基线、规则审计、同机 RankHub 对标、分 checkpoint 实现与数据指纹记录见 `ANDROID_CONVERGENCE_PROGRESS.md`。

- iOS 已冻结：本轮没有修改、构建或验证 iOS，也没有继续扩张跨平台能力。
- Android 底栏由“首页 / 记录 / 导入 / 谱面 / 工具”收束为“首页 / 导入 / 谱面 / 工具”。删除独立记录入口及重复全量成绩浏览器；首页只保留 B50、B35/B15 跳转和分析快捷入口。
- “谱面”成为唯一全量浏览器和唯一筛选状态源。万能搜索、`13 / 13+ / 13.3`、精确定数、定数范围滑块、难度/版本/分区/游玩状态、成绩范围、Rank、FC、FS、类型与排序全部保留；高频条件常显，完整条件渐进进入高级 sheet。
- 建立统一版本/牌子目录，修正 `MiLK PLUS → 雪`、`FiNALE → 輝` 和小版本边界/官方排除集合。实机 `輝将` 为 180 张并包含 `PANDORA PARADOXXX`；版本页改为名称、相关版本、牌子简称和内部范围的可读对照。
- 简繁搜索改为离线 OpenCC 字典驱动的最长短语 trie + 字符映射；“华火职人”与“華火職人”双向命中。MASTER / Re:MASTER 颜色统一到同一紫色家族的共享 token。
- 谱面索引在根层预热、按内容去重并合并启动输入；整份简繁语料一次转换。旧基线从首页进入谱面约 2.117 秒才稳定，最终同机预热后约 236–270ms 显示完整结果，达到 RankHub 约 283ms 的只读对标水平；高级筛选点击先显示首帧选中反馈，再挂载完整弹层。
- 失分计算器默认支持选择谱面自动读取完整 Note 数，同时保留手动模式；实机 PANDORA MASTER 自动得到 1309 Note 及五类物量。工具页区段和判定选项改为换行，手机宽度不再裁切关键 chip。
- 最终回归：46 suites / 223 tests / 0 failures / errors / skipped；Debug APK 22,932,701 bytes，SHA-256 `58DF8B62C18692E7CE55BB8F05BBA60BD7D3FD2A4E2F14384B9F9696D2886AD3`。
- 最终真机只执行 `adb install -r`；没有卸载、清数据或删除旧文件。SQLite 完整性、schema 6、1,628 条成绩、9,985 条隔离记录、30 个导入批次、1 个 Rating 历史、5 个保存页及全部语义 SHA-256 均与本轮修改前基线一致；旧 Token 与 7 个 HTML 的元数据组合指纹也一致，内容未读取。
